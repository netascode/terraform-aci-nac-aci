...
class Mso:
    ...
    def login(self):
        ...
        if resp.status_code not in [200, 201]:
            return "MSO login failed, status code: {}, response: {}.".format(
    ...
    def logout(self):
        ...
        if resp.status_code != 200:
            return "MSO logout failed, status code: {}".format(resp.status_code)
        return None
    ...
    def post_or_put(self, path: str, data: str, method: str = ""):
        ...
        if resp.status_code not in [200, 201]:
            if "Cannot run program" in resp.text and resp.status_code == 400:
                return ""
            return "MSO {} failed, status code: {}, response: {}.".format(
...
def get(self, path: str) -> str:
    ...
    if resp.status_code != 200:
        return "MSO GET failed, status code: {}, response: {}.".format(
            resp.status_code, resp.text
        )
    return ""
