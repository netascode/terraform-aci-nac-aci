...
locals {
  fabric_leaf_interface_selectors = flatten([
    for node in local.nodes : [
      for interface in try(node.interfaces, []) : {
        key               = format("%s/%s/%s", node.id, try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port)
        name              = replace(format("%s:%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port), "/^(?P<mod>.+):(?P<port>.+)$/", replace(replace(try(local.fabric_policies.leaf_interface_selector_name, local.defaults.apic.fabric_policies.leaf_interface_selector_name), "\\g<mod>", "$${mod}"), "\\g<port>", "$${port}"))
        description       = try(local.apic.interface_selector_description, local.defaults.apic.interface_selector_description) ? try(interface.description, "") : ""
  ...
  fabric_leaf_sub_interface_selectors = flatten([
    for node in local.nodes : [
      for interface in try(node.interfaces, []) : [
        for sub in try(interface.sub_ports, []) : {
          key               = format("%s/%s/%s/%s", node.id, try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, sub.port)
          name              = replace(format("%s:%s:%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, sub.port), "/^(?P<mod>.+):(?P<port>.+):(?P<sport>.+)$/", replace(replace(replace(try(local.fabric_policies.leaf_interface_selector_sub_port_name, local.defaults.apic.fabric_policies.leaf_interface_selector_sub_port_name), "\\g<mod>", "$${mod}"), "\\g<port>", "$${port}"), "\\g<sport>", "$${sport}"))
          description       = try(local.apic.interface_selector_description, local.defaults.apic.interface_selector_description) ? try(sub.description, "") : ""
          interface_profile = replace("${node.id}:${node.name}", "/^(?P<id>.+):(?P<name>.+)$/", replace(replace(try(local.fabric_policies.leaf_interface_profile_name, local.defaults.apic.fabric_policies.leaf_interface_profile_name), "\\g<id>", "$${id}"), "\\g<name>", "$${name}"))
...
locals {
  fabric_spine_interface_selectors = flatten([
    for node in local.nodes : [
      for interface in try(node.interfaces, []) : {
        key               = format("%s/%s/%s", node.id, try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port)
        name              = replace(format("%s:%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port), "/^(?P<mod>.+):(?P<port>.+)$/", replace(replace(try(local.fabric_policies.spine_interface_selector_name, local.defaults.apic.fabric_policies.spine_interface_selector_name), "\\g<mod>", "$${mod}"), "\\g<port>", "$${port}"))
        description       = try(local.apic.interface_selector_description, local.defaults.apic.interface_selector_description) ? try(interface.description, "") : ""
        interface_profile = replace("${node.id}:${node.name}", "/^(?P<id>.+):(?P<name>.+)$/", replace(replace(try(local.fabric_policies.spine_interface_profile_name, local.defaults.apic.fabric_policies.spine_interface_profile_name), "\\g<id>", "$${id}"), "\\g<name>", "$${name}"))
        policy_group      = try("${interface.policy_group}${local.defaults.apic.fabric_policies.spine_interface_policy_groups.name_suffix}", "")
        port_blocks = [{
          description = try(interface.description, "")
          name        = format("%s-%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port)
          from_module = try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module)
...
module "aci_fabric_spine_interface_selector_auto" {
  source = "./modules/terraform-aci-fabric-spine-interface-selector"

  for_each          = { for selector in local.fabric_spine_interface_selectors : selector.key => selector if local.modules.aci_fabric_spine_interface_selector && var.manage_interface_policies }
  name              = each.value.name
  description       = each.value.description
  interface_profile = each.value.interface_profile
  policy_group      = each.value.policy_group
...
locals {
  fabric_spine_sub_interface_selectors = flatten([
    for node in local.nodes : [
      for interface in try(node.interfaces, []) : [
        for sub in try(interface.sub_ports, []) : {
          key               = format("%s/%s/%s/%s", node.id, try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, sub.port)
          name              = replace(format("%s:%s:%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, sub.port), "/^(?P<mod>.+):(?P<port>.+):(?P<sport>.+)$/", replace(replace(replace(try(local.fabric_policies.spine_interface_selector_sub_port_name, local.defaults.apic.fabric_policies.spine_interface_selector_sub_port_name), "\\g<mod>", "$${mod}"), "\\g<port>", "$${port}"), "\\g<sport>", "$${sport}"))
          description       = try(local.apic.interface_selector_description, local.defaults.apic.interface_selector_description) ? try(sub.description, "") : ""
          interface_profile = replace("${node.id}:${node.name}", "/^(?P<id>.+):(?P<name>.+)$/", replace(replace(try(local.fabric_policies.spine_interface_profile_name, local.defaults.apic.fabric_policies.spine_interface_profile_name), "\\g<id>", "$${id}"), "\\g<name>", "$${name}"))
          policy_group      = try("${sub.policy_group}${local.defaults.apic.fabric_policies.spine_interface_policy_groups.name_suffix}", "")
          sub_port_blocks = [{
            description   = try(sub.description, "")
            name          = format("%s-%s-%s", try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, sub.port)
            from_module   = try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module)
...
module "aci_fabric_spine_interface_selector_sub_auto" {
  source = "./modules/terraform-aci-fabric-spine-interface-selector"

  for_each          = { for selector in local.fabric_spine_sub_interface_selectors : selector.key => selector if local.modules.aci_fabric_spine_interface_selector && var.manage_interface_policies }
  name              = each.value.name
  description       = each.value.description
  interface_profile = each.value.interface_profile
  policy_group      = each.value.policy_group
...
locals {
  new_spine_fabric_subinterface_configuration = flatten([
    for node in local.nodes : [
      for interface in try(node.interfaces, []) : [
        for subinterface in try(interface.sub_ports, []) : {
          key          = format("%s/%s/%s/%s", node.id, try(interface.module, local.defaults.apic.interface_policies.nodes.interfaces.module), interface.port, subinterface.port)
          ...
          port         = interface.port
          sub_port     = subinterface.port
          policy_group = try("${subinterface.policy_group}${local.defaults.apic.fabric_policies.spine_interface_policy_groups.name_suffix}", "system-ports-default")
          description  = try(subinterface.description, "")
          shutdown     = try(subinterface.shutdown, local.defaults.apic.interface_policies.nodes.interfaces.sub_ports.shutdown)
          role         = node.role
        }
...
module "aci_spine_fabric_interface_configuration_sub" {
  source = "./modules/terraform-aci-fabric-interface-configuration"

  for_each     = { for int in local.new_spine_fabric_subinterface_configuration : int.key => int if local.modules.aci_fabric_interface_configuration && var.manage_interface_policies }
  node_id      = each.value.node_id
  module       = each.value.module
  port         = each.value.port
  sub_port     = each.value.sub_port
