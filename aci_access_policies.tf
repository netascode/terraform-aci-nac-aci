...
module "aci_routed_domain" {
  ...
  vlan_pool_allocation = try(each.value.vlan_pool, "") != "" ? [for vp in try(local.access_policies.vlan_pools, []) : try(vp.allocation, local.defaults.apic.access_policies.vlan_pools.allocation) if vp.name == each.value.vlan_pool][0] : null
  security_domains     = try(each.value.security_domains, [])
  depends_on = [
    module.aci_vlan_pool,
  ]
