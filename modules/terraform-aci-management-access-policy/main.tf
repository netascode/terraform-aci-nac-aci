locals {
  ...
  # Only ciphers the user explicitly set (non-null) are managed; APIC's cipher
  # vocabulary differs per release, so there is no safe always-on default set.
  https_ssl_ciphers = { for k, v in {
    ...
    "CHACHA20"                      = var.https_ssl_cipher_chacha20
    "DHE-RSA-AES128-SHA"            = var.https_ssl_cipher_dhe_rsa_aes128_sha
    "DHE-RSA-AES256-SHA"            = var.https_ssl_cipher_dhe_rsa_aes256_sha
    "ECDHE-RSA-AES128-SHA"          = var.https_ssl_cipher_ecdhe_rsa_aes128_sha
    "ECDHE-RSA-AES256-SHA"          = var.https_ssl_cipher_ecdhe_rsa_aes256_sha
    "ECDSA"                         = var.https_ssl_cipher_ecdsa
    "EDH+aRSA"                      = var.https_ssl_cipher_edh_arsa
    "EECDH"                         = var.https_ssl_cipher_eecdh
    "EECDH+aRSA+AESGCM"             = var.https_ssl_cipher_eecdh_arsa_aesgcm
    "EECDH+aRSA+SHA256"             = var.https_ssl_cipher_eecdh_arsa_sha256
    "EECDH+aRSA+SHA384"             = var.https_ssl_cipher_eecdh_arsa_sha384
  } : k => v if v != null }
}
