...
data "aci_rest_managed" "spanDestGrp" {
  dn = "uni/infra/destgrp-TEST_PATH_SUBPORT"
...
data "aci_rest_managed" "spanDest" {
  dn = "${data.aci_rest_managed.spanDestGrp.id}/dest-TEST_PATH_SUBPORT"
...
data "aci_rest_managed" "spanRsDestPathEp" {
  dn = "${data.aci_rest_managed.spanDest.id}/rsdestPathEp-[topology/pod-2/paths-101/pathep-[eth2/10/5]]"
...
resource "test_assertions" "spanRsDestPathEp" {
  ...
  equal "tDn" {
    ...
    want        = "topology/pod-2/paths-101/pathep-[eth2/10/5]"
  }
}
