...
| <a name="input_vcenters"></a> [vcenters](#input\_vcenters) | List of vCenter hosts. Choices `dvs_version`: `unmanaged` or a version in the format `NUMBER.NUMBER`. Default value `dvs_version`: `unmanaged`. Default value `statistics`: false. Allowed values `mgmt_epg_type`: `inb`, `oob`. Default value `mgmt_epg_type`: `inb`. | <pre>list(object({<br/>    name              = string<br/>    hostname_ip       = string<br/>    datacenter        = string<br/>    credential_policy = optional(string)<br/>    dvs_version       = optional(string, "unmanaged")<br/>    statistics        = optional(bool, false)<br/>    mgmt_epg_type     = optional(string, "inb")<br/>    mgmt_epg_name     = optional(string)<br/>  }))</pre> | `[]` | no |
