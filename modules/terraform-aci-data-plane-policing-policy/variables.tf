...
variable "tenant" {
  ...
  validation {
    condition     = var.tenant == null ? true : can(regex("^[a-zA-Z0-9_.-]{0,63}$", var.tenant))
...
variable "conform_mark_dscp" {
  type        = string
  description = "Conform Mark Dscp. Allowed values are `unspecified` or a number between 0 and 63."
  default     = "unspecified"
