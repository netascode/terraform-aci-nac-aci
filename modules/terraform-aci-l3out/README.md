...
| <a name="input_route_maps_name"></a> [route\_maps\_name](#input\_route\_maps\_name) | Route map names. | <pre>list(object({<br>    name        = string<br>    description = optional(string, "")<br>    type        = string<br>  }))</pre> | `[]` | no |
| <a name="input_route_maps_description"></a> [route\_maps\_description](#input\_route\_maps\_description) | Route maps description. | `string` | `""` | no |
| <a name="input_route_maps_type"></a> [route\_maps\_type](#input\_route\_maps\_type) | Route maps type. Choices: `combinable`, `global`. | `string` | `"combinable"` | no |
| <a name="input_route_maps_contexts"></a> [route\_maps\_contexts](#input\_route\_maps\_contexts) | List of route map contexts. Choices `action`: `permit`, `deny`. Default value `action`: `permit`. Allowed values `order`: 0-9. Default value `order`: 0. | <pre>list(object({<br>    name        = string<br>    description = optional(string, "")<br>    type        = string<br>    action      = optional(string, "permit")<br>    order       = optional(number, 0)<br>    set_rule    = optional(string)<br>    match_rules = optional(list(string), [])<br>  }))</pre> | `[]` | no |
