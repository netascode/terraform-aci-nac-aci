...
Verify MCP Global Instance
    ...
    Should Be Equal Value Json String   ${r.json()}    $..mcpInstPol.attributes.loopProtectAct   {% if apic.access_policies.mcp.action | default(defaults.apic.access_policies.mcp.action) | cisco.aac.aac_bool("enabled") == "enabled" %}port-disable{% elif apic.access_policies.mcp.action | default(defaults.apic.access_policies.mcp.action) | cisco.aac.aac_bool("enabled") == "disabled" %}""{% endif %} 
