...
Verify VSPAN Session {{ vspan_name }}
    GET  "/api/mo/uni/infra/vsrcgrp-{{ vspan_name }}.json?rsp-subtree=full"
    String   $..spanVSrcGrp.attributes.name   {{ vspan_name }}
    String   $..spanVSrcGrp.attributes.descr   {{ vspan.description | default() }}
    String   $..spanVSrcGrp.attributes.adminSt   {% if vspan.admin_state | default(defaults.apic.access_policies.vspan.sessions.admin_state) | cisco.aac.aac_bool("enabled") == "enabled" %}start{% else %}stop{% endif %}
...
Verify VSPAN Session {{ vspan_name }} Source {{ source_name }}
    ...
    String   ${source}.attributes.descr   {{ source.description | default() }}
    String   ${source}.attributes.dir   {{ source.direction | default(defaults.apic.access_policies.vspan.sessions.sources.direction) }}
...
{% set pod = path.pod_id | default(((apic.node_policies | default()) | community.general.json_query(query))[0] | default('1')) %}
    ...
    String   ${path}.attributes.tDn   topology/pod-{{ pod }}/paths-{{ path.node_id }}/pathep-[eth{{ path.module | default(defaults.apic.access_policies.vspan.sessions.sources.access_paths.module) }}/{{ path.port }}]
{% else %}
...
{% set application_profile_name = source.application_profile ~ defaults.apic.tenants.application_profiles.name_suffix %}      
{% set endpoint_group_name = source.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    String   ${source}..spanRsSrcToVPort.attributes.tDn   uni/tn-{{ source.tenant }}/ap-{{ application_profile_name }}/epg-{{ endpoint_group_name }}/cep-{{ source.endpoint}}                           
{% endif %}

{% endfor %}

{% set destination_name = vspan.destination.name ~ defaults.apic.access_policies.vspan.destination_groups.name_suffix %}
    String   $..spanVSrcGrp.children..spanSpanLbl.attributes.name   {{ destination_name }}
    String   $..spanVSrcGrp.children..spanSpanLbl.attributes.descr   {{ vspan.destination.description | default() }}
{% endfor %}
