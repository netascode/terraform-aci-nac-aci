...
Verify QoS COS presevation status
    ...
    String   $..qosInstPol.attributes.ctrl   "{% if apic.access_policies.qos.preserve_cos | default(defaults.apic.access_policies.qos.preserve_cos) | cisco.aac.aac_bool("enabled") == "enabled" %}dot1p-preserve{% elif apic.access_policies.qos.preserve_cos | default(defaults.apic.access_policies.qos.preserve_cos) | cisco.aac.aac_bool("enabled") == "disabled" %}{% endif %}"
...
Verify QoS Class Level {{ level }}
    ...
    String   ${level}..qosClass.attributes.admin   {{ qos_class.admin_state | default(default_qos_class.admin_state) | cisco.aac.aac_bool("enabled") }}
    ...
    String   ${level}..qosPfcPol.attributes.adminSt   {% if qos_class.pfc_state | default(default_qos_class.pfc_state) | cisco.aac.aac_bool("enabled") == "enabled" %}yes{% else %}no{% endif %}
    ...
    String   ${level}..qosCong.attributes.ecn   {{ qos_class.ecn | default(default_qos_class.ecn) | cisco.aac.aac_bool("enabled") }}
    String   ${level}..qosCong.attributes.forwardNonEcn   {{ qos_class.forward_non_ecn | default(default_qos_class.forward_non_ecn) | cisco.aac.aac_bool("enabled") }}
