...
Verify Tenant SPAN Source Group {{ span_grp_name }} Source {{ source.name }}
    ...
    String   ${src}.attributes.name   {{ source.name  }}
    String   ${src}.attributes.descr   {{ source.description | default()}}
    String   ${src}.attributes.dir   {{ source.direction | default(defaults.apic.tenants.policies.span.source_groups.sources.direction ) }}
...
{% set endpoint_group_name = source.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}                                    
    String   ${src}..spanRsSrcToEpg.attributes.tDn   uni/tn-{{ tenant.name }}/ap-{{ application_profile_name }}/epg-{{ endpoint_group_name }}
{% endif %} 
