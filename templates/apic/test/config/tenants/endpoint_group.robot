...
Verify Endpoint Group {{ epg_name }}
    ...
    Should Be Equal Value Json String   ${r.json()}   $..fvAEPg.attributes.fwdCtrl  {% if epg.proxy_arp | default(defaults.apic.tenants.application_profiles.endpoint_groups.proxy_arp) %}proxy-arp{% else %}""{% endif %} 
