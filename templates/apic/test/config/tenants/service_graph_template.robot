...
{% set dev = (apic | json_query(query))[0] %}
...
Verify Service Graph Template {{ sgt_name }}
    ...
    String   $..vnsAbsNode.attributes.isCopy   {{ dev.copy_device | default(defaults.apic.tenants.services.l4l7_devices.copy_device) }}
    String   $..vnsAbsNode.attributes.managed   {{ dev.managed | default(defaults.apic.tenants.services.l4l7_devices.managed) }}
    String   $..vnsAbsNode.attributes.routingMode   {{ 'Redirect' if sgt.redirect | default(defaults.apic.tenants.services.service_graph_templates.redirect) == 'enabled' else 'unspecified' }} 
    String   $..vnsAbsNode.attributes.shareEncap   {{ 'yes' if sgt.share_encapsulation | default(defaults.apic.tenants.services.service_graph_templates.share_encapsulation)  == 'enabled' else 'no' }}
