...
Verify DHCP Relay Policy {{ policy_name }}
    GET   "/api/node/mo/uni/tn-{{ tenant.name }}/relayp-{{ policy_name }}.json?rsp-subtree=full"
    String   $..dhcpRelayP.attributes.name   {{ policy_name }}
    String   $..dhcpRelayP.attributes.descr  {{ policy.description | default() }}
...
Verify DHCP Relay Policy {{ policy_name }} Provider {{ provider.ip }}
    ...
    String   ${provider}..dhcpRsProv.attributes.addr   {{ provider.ip }}
...
{% set epg_name = provider.endpoint_group ~ defaults.apic.tenants.application_profiles.endpoint_groups.name_suffix %}
    String   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/ap-{{ ap_name }}/epg-{{ epg_name }}
...
{% set eepg_name = provider.external_endpoint_group ~ defaults.apic.tenants.l3outs.external_endpoint_groups.name_suffix %}
    String   ${provider}..dhcpRsProv.attributes.tDn   uni/tn-{{ provider.tenant | default(tenant.name) }}/out-{{ l3out_name }}/instP-{{ eepg_name }}
{% endif %}
