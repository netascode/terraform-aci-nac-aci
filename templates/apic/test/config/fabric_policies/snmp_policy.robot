...
Verify SNMP Policy '{{ snmp_policy_name }}'
    ...
    String   $..snmpPol.attributes.contact   {{ policy.contact | default(defaults.apic.fabric_policies.pod_policies.snmp_policies.contact) }}
...
Verify SNMP Policy '{{ snmp_policy_name }}' User '{{ user.name }}'
    ...
    String    ${usr}..snmpUserP.attributes.name   {{ user.name }}
    String    ${usr}..snmpUserP.attributes.authType   {{ user.authorization_type | default(defaults.apic.fabric_policies.pod_policies.snmp_policies.users.authorization_type) }}
    String    ${usr}..snmpUserP.attributes.privType   {{ user.privacy_type | default(defaults.apic.fabric_policies.pod_policies.snmp_policies.users.privacy_type)}}
{% endfor %}
...
Verify SNMP Policy '{{ snmp_policy_name }}' Client Group '{{ snmp_client_name }}'
    ...
    String    ${client}..snmpClientGrpP.attributes.name   {{ snmp_client_name }}
    ...
    {% endif %}
    String    ${client}..snmpClientGrpP.attributes.epgDn   ${mgmt_epg}
...
Verify SNMP Policy '{{ snmp_policy_name }}' Client Group '{{ snmp_client_name }}' Entry '{{ client_entry.name }}'
    ...
    String    ${ent}..snmpClientP.attributes.name   {{ client_entry.name }}
    String    ${ent}..snmpClientP.attributes.addr   {{ client_entry.ip }}
{% endfor %}
...
Verify SNMP Policy '{{ snmp_policy_name }}' Community '{{ community }}'
    ...
    String    ${comm}..snmpCommunityP.attributes.name   {{ community }}
{% endfor %}
...
Verify SNMP Policy '{{ snmp_policy_name }}' Trap Forward Server '{{ trap_forwarder.ip }}'
    ...
    String    ${tfs}..snmpTrapFwdServerP.attributes.addr   {{ trap_forwarder.ip }}
    String    ${tfs}..snmpTrapFwdServerP.attributes.port   {{ trap_forwarder.port | default(defaults.apic.fabric_policies.pod_policies.snmp_policies.trap_forwarders.port) }}
{% endfor %}
