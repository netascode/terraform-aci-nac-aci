...
Verify Spine Switch Profile {{ spine_switch_profile_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}ap_spine_switch_profile_{{ spine_switch_profile_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Spine Switch Profile {{ spine_switch_profile_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}ap_spine_switch_profile_{{ spine_switch_profile_name }}_faults.json'))   modules=json
