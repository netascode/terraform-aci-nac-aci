*** Settings ***
Documentation   Verify Access Leaf Interface Profile Health
...
Verify Leaf Interface Profile {{ leaf_interface_profile_name }} Faults
    ${r}=   GET On Session   apic   /api/mo/uni/infra/accportprof-{{ leaf_interface_profile_name }}/fltCnts.json
...
Verify Application Profile {{ node_name }} Faults Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ leaf_interface_profile_name }}/node-{{ node_name }}/fltCnts.json
    ${critical}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.crit
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{json}=    Create Dictionary   critical=${critical}[0]   major=${major}[0]   minor=${minor}[0]
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ leaf_interface_profile_name }}_node_{{ node_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Application Profile {{ node_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ leaf_interface_profile_name }}_node_{{ node_name }}_faults.json'))   modules=json
...
Verify Application Profile {{ node_name }} Health
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ leaf_interface_profile_name }}/node-{{ node_name }}/health.json
...
Verify Application Profile {{ node_name }} Health Pre-Check
    ...
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ leaf_interface_profile_name }}_node_{{ node_name }}_health.json', 'w'))   modules=json
{% endif %}
...
Verify Application Profile {{ node_name }} Health Post-Check
    ...
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ leaf_interface_profile_name }}/node-{{ node_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ leaf_interface_profile_name }}_node_{{ node_name }}_health.json'))   modules=json
