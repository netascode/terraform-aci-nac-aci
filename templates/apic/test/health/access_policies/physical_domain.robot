...
{% if domain.expected_state.maximum_critical_faults is defined %}
    Run Keyword If   ${critical}[0] > {{ domain.expected_state.maximum_critical_faults }}   Run Keyword And Continue On Failure
...
Verify Physical Domain {{ domain_name }} Faults Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/phys-{{ domain_name }}/fltCnts.json
    ${critical}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.crit
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ...
    evaluate   json.dump($json, open('${STATE_PATH}physical_domain_{{ domain.name }}_faults.json', 'w'))   modules=json
{% endif %}

{% if 'post-check' in robot_include_tags | default() %}
Verify Physical Domain {{ domain_name }} Faults Post-Check
    [Tags]   post-check
    ${r}=   GET On Session   apic   /api/mo/uni/phys-{{ domain_name }}/fltCnts.json
    ${critical}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.crit
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}physical_domain_{{ domain.name }}_faults.json'))   modules=json
