...
Verify Leaf Switch Profile {{ leaf_switch_profile_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}ap_leaf_switch_profile_{{ leaf_switch_profile_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Leaf Switch Profile {{ leaf_switch_profile_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}ap_leaf_switch_profile_{{ leaf_switch_profile_name }}_faults.json'))   modules=json
