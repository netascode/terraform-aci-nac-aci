...
Verify SPAN Destination Group {{ span_name }} Faults
    ${r}=   GET On Session   apic   /api/mo/uni/infra/destgrp-{{ span_name }}/fltCnts.json
...
Verify SPAN Destination Group {{ span_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}ap_span_destination_group_{{ span_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify SPAN Destination Group {{ span_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}ap_span_destination_group_{{ span_name }}_faults.json'))   modules=json
