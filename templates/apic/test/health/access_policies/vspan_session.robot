...
Verify VSPAN Session {{ vspan_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}vspan_session_{{ vspan.name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify VSPAN Session {{ vspan_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}vspan_session_{{ vspan.name }}_faults.json'))   modules=json
