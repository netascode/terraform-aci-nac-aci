*** Settings ***
Documentation   Verify LDAP Provider Health
Suite Setup     Login APIC
Default Tags    apic   day0   health   fabric_policies   non-critical
Resource        ../../apic_common.resource

*** Test Cases ***
{% for prov in apic.fabric_policies.aaa.ldprov.providers | default([]) %}
...
Verify LDAP Provider {{ prov.hostname_ip }} Faults
    ${r}=   GET On Session   apic   /api/mo/uni/userext/ldapext/ldapprovider-{{ prov.hostname_ip }}/fltCnts.json
...
Verify LDAP Provider {{ prov.hostname_ip }} Faults Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/userext/ldapext/ldapprovider-{{ prov.hostname_ip }}/fltCnts.json
    ${critical}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.crit
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_ldapprovider_{{ prov.hostname_ip }}_faults.json', 'w'))   modules=json
{% endif %}

{% if 'post-check' in robot_include_tags | default() %}
Verify LDAP Provider {{ prov.hostname_ip }} Faults Post-Check
    [Tags]   post-check
    ${r}=   GET On Session   apic   /api/mo/uni/userext/ldapext/ldapprovider-{{ prov.hostname_ip }}/fltCnts.json
    ${critical}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.crit
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_ldapprovider_{{ prov.hostname_ip }}_faults.json'))   modules=json
...
Verify LDAP Provider {{ prov.hostname_ip }} Health
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/ldapprovider-{{ prov.hostname_ip }}/health.json
...
{% if 'pre-check' in robot_include_tags | default() %}
Verify LDAP Provider {{ prov.hostname_ip }} Health Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/ldapprovider-{{ prov.hostname_ip }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{json}=    Create Dictionary   health=${health}[0]
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_ldapprovider_{{ prov.hostname_ip }}_health.json', 'w'))   modules=json
{% endif %}

{% if 'post-check' in robot_include_tags | default() %}
Verify LDAP Provider {{ prov.hostname_ip }} Health Post-Check
    [Tags]   post-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/ldapprovider-{{ prov.hostname_ip }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_ldapprovider_{{ prov.hostname_ip }}_health.json'))   modules=json
