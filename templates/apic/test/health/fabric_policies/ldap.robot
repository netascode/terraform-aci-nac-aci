...
Verify LDAP Provider {{ prov.hostname_ip }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}ldap_{{ prov.hostname_ip }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify LDAP Provider {{ prov.hostname_ip }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}ldap_{{ prov.hostname_ip }}_faults.json'))   modules=json
