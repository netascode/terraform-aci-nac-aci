...
{% if pod.expected_state.maximum_critical_faults is defined %}
    ...
    ...   Fail  "{{ pod_profile_name }} has ${critical}[0] critical faults"
{% endif %}
{% if pod.expected_state.maximum_major_faults is defined %}
    ...
    ...   Fail  "{{ pod_profile_name }} has ${major}[0] major faults"
{% endif %}
{% if pod.expected_state.maximum_minor_faults is defined %}
    ...
    ...   Fail  "{{ pod_profile_name }} has ${minor}[0] minor faults"
{% endif %}
{% endif %}
...
Verify Pod Profile {{ pod_profile_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_podprof_{{ pod_profile_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Pod Profile {{ pod_profile_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_podprof_{{ pod_profile_name }}_faults.json'))   modules=json
...
{% if pod.expected_state.minimum_health is defined %}
Verify Pod Profile {{ pod_profile_name }} Health
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/podprof-{{ pod_profile_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    Run Keyword If   ${health}[0] < {{ pod.expected_state.minimum_health }}   Run Keyword And Continue On Failure
    ...   Fail  "{{ pod_profile_name }} health score: ${health}[0]"
{% endif %}

{% if 'pre-check' in robot_include_tags | default() %}
Verify Pod Profile {{ pod_profile_name }} Health Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/podprof-{{ pod_profile_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{json}=    Create Dictionary   health=${health}[0]
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_podprof_{{ pod_profile_name }}_health.json', 'w'))   modules=json
{% endif %}

{% if 'post-check' in robot_include_tags | default() %}
Verify Pod Profile {{ pod_profile_name }} Health Post-Check
    [Tags]   post-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/podprof-{{ pod_profile_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_podprof_{{ pod_profile_name }}_health.json'))   modules=json
    Run Keyword If   ${health}[0] < ${previous["health"]}   Run Keyword And Continue On Failure
    ...   Fail  "{{ pod_profile_name }} health score degraded from ${previous["health"]} to ${health}[0]"
{% endif %}
