...
Verify SPAN Source Group {{ span_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_srcgrp_{{ span_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify SPAN Source Group {{ span_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_srcgrp_{{ span_name }}_faults.json'))   modules=json
...
Verify SPAN Source Group {{ span_name }} Health
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/srcgrp-{{ span_name }}/health.json
...
{% if 'pre-check' in robot_include_tags | default() %}
Verify SPAN Source Group {{ span_name }} Health Pre-Check
    [Tags]   pre-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/srcgrp-{{ span_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{json}=    Create Dictionary   health=${health}[0]
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_srcgrp_{{ span_name }}_health.json', 'w'))   modules=json
{% endif %}

{% if 'post-check' in robot_include_tags | default() %}
Verify SPAN Source Group {{ span_name }} Health Post-Check
    [Tags]   post-check
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/srcgrp-{{ span_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_srcgrp_{{ span_name }}_health.json'))   modules=json
