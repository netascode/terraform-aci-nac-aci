...
Verify ND RA Prefix Policy {{ policy_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_nd_ra_prefix_policy_{{ policy_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify ND RA Prefix Policy {{ policy_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_nd_ra_prefix_policy_{{ policy_name }}_faults.json'))   modules=json
