...
Verify Inband Endpoint Group {{ epg_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_inb_{{ epg_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Inband Endpoint Group {{ epg_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCountsWithDetails.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_inb_{{ epg_name }}_faults.json'))   modules=json
...
Verify Inband Endpoint Group {{ epg_name }} Health Pre-Check
    ...
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_inb_{{ epg_name }}_health.json', 'w'))   modules=json
{% endif %}
...
Verify Inband Endpoint Group {{ epg_name }} Health Post-Check
    ...
    ${r}=   GET On Session   apic   /api/mo/uni/tn-mgmt/mgmtp-default/inb-{{ epg_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_inb_{{ epg_name }}_health.json'))   modules=json
