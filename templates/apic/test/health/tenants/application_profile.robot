...
Verify Application Profile {{ ap_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_application_profile_{{ ap_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Application Profile {{ ap_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_application_profile_{{ ap_name }}_faults.json'))   modules=json
...
Verify Application Profile {{ ap_name }} Health Pre-Check
    ...
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_application_profile_{{ ap_name }}_health.json', 'w'))   modules=json
{% endif %}
...
Verify Application Profile {{ ap_name }} Health Post-Check
    ...
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/ap-{{ ap_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_application_profile_{{ ap_name }}_health.json'))   modules=json
