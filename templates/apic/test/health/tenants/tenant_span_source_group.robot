...
Verify Tenant SPAN Source Group {{ span_src_grp_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_span_source_group_{{ span_src_grp_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Tenant SPAN Source Group {{ span_src_grp_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_span_source_group_{{ span_src_grp_name }}_faults.json'))   modules=json
