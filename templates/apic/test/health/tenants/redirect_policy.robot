...
Verify Redirect Policy {{ pol_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_redirect_policy_{{ pol_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Redirect Policy {{ pol_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_redirect_policy_{{ pol_name }}_faults.json'))   modules=json
