...
Verify Bridge Domain {{ bd_name }} Faults Pre-Check
    ...
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_bridge_domain_{{ bd_name }}_faults.json', 'w'))   modules=json
{% endif %}
...
Verify Bridge Domain {{ bd_name }} Faults Post-Check
    ...
    ${major}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.maj
    ${minor}=   Get Value From Json   ${r.json()}   $..faultCounts.attributes.minor
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_bridge_domain_{{ bd_name }}_faults.json'))   modules=json
...
Verify Bridge Domain {{ bd_name }} Health Pre-Check
    ...
    Create Directory   ${STATE_PATH}
    evaluate   json.dump($json, open('${STATE_PATH}tenant_{{ tenant.name }}_bridge_domain_{{ bd_name }}_health.json', 'w'))   modules=json
{% endif %}
...
Verify Bridge Domain {{ bd_name }} Health Post-Check
    ...
    ${r}=   GET On Session   apic   /api/mo/uni/tn-{{ tenant.name }}/BD-{{ bd_name }}/health.json
    ${health}=   Get Value From Json   ${r.json()}   $..healthInst.attributes.cur
    &{previous}=   evaluate   json.load(open('${STATE_PATH}tenant_{{ tenant.name }}_bridge_domain_{{ bd_name }}_health.json'))   modules=json
