              ...
              for node in try(np.nodes, []) : [
                for static_route in try(node.static_routes, []) : [
                  for next_hop in try(static_route.next_hops, []) : {
                    ...
                    tenant          = tenant.name
                    name            = try("${l3out.vrf}${local.defaults.apic.tenants.vrfs.name_suffix}_${next_hop.ip}${local.defaults.apic.tenants.policies.track_lists.name_suffix}", null)
                    ...
                    track_members   = try(["${l3out.vrf}${local.defaults.apic.tenants.vrfs.name_suffix}_${next_hop.ip}${local.defaults.apic.tenants.policies.track_members.name_suffix}"], [])
