  ...
  service_graph_templates = flatten([
    for tenant in local.tenants : [
      for sgt in try(tenant.services.service_graph_templates, []) : {
        ...
        device_function         = length([for d in local.l4l7_devices : d if d.tenant == tenant.name || d.tenant == try(sgt.device.tenant, tenant.name)]) > 0 ? [for device in local.l4l7_devices : try(device.function, []) if device.name == sgt.device.name && (device.tenant == tenant.name || device.tenant == try(sgt.device.tenant, tenant.name))][0] : "None"
        device_copy             = length([for d in local.l4l7_devices : d if d.tenant == tenant.name || d.tenant == try(sgt.device.tenant, tenant.name)]) > 0 ? [for device in local.l4l7_devices : try(device.copy_device, []) if device.name == sgt.device.name && (device.tenant == tenant.name || device.tenant == try(sgt.device.tenant, tenant.name))][0] : false
        device_managed          = length([for d in local.l4l7_devices : d if d.tenant == tenant.name || d.tenant == try(sgt.device.tenant, tenant.name)]) > 0 ? [for device in local.l4l7_devices : try(device.managed, []) if device.name == sgt.device.name && (device.tenant == tenant.name || device.tenant == try(sgt.device.tenant, tenant.name))][0] : false
