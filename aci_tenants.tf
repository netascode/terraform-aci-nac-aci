  ...
  device_selection_policies = flatten([
    for tenant in local.tenants : [
      for dsp in try(tenant.services.device_selection_policies, []) : {
        ...
        copy_logical_interfafce                                 = "${dsp.copy_service.logical_interface}${local.defaults.apic.tenants.services.l4l7_devices.logical_interfaces.name_suffix}"
...
module "aci_device_selection_policy" {
  ...
  copy_logical_interfafce                                 = each.value.copy_logical_interfafce
