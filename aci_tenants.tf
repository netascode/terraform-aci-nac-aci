  ...
  nd_interface_policies = flatten([
    for tenant in local.tenants : [
      for policy in try(tenant.policies.nd_interface_policies, []) : {
        ...
        nud_retransmit_base      = try(policy.nud_retransmit_base, null)
        nud_retransmit_interval  = try(policy.nud_retransmit_interval, null)
        nud_retransmit_count     = try(policy.nud_retransmit_count, null)
