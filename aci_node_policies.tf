module "aci_firmware_group" {
  ...
  for_each = { for np in try(local.node_policies.update_groups, {}) : np.name => np if try(local.modules.aci_firmware_group, true) && var.manage_node_policies }
...
module "aci_maintenance_group" {
  ...
  for_each = { for np in try(local.node_policies.update_groups, {}) : np.name => np if try(local.modules.aci_maintenance_group, true) && var.manage_node_policies }
  name     = "${each.value.name}${local.defaults.apic.node_policies.update_groups.name_suffix}"
  node_ids = [for node in try(local.node_policies.nodes, []) : node.id if try(node.update_group, "") == each.value.name]
}
...
module "aci_vpc_group" {
  ...
  count = try(local.modules.aci_vpc_group, true) == true && var.manage_node_policies ? 1 : 0
...
module "aci_inband_node_address" {
  ...
  for_each       = { for node in try(local.node_policies.nodes, []) : node.id => node if(try(node.inb_address, null) != null || try(node.inb_v6_address, null) != null) && try(local.modules.aci_inband_node_address, true) && var.manage_node_policies }
  node_id        = each.value.id
  pod_id         = try(each.value.pod, local.defaults.apic.node_policies.nodes.pod)
...
module "aci_oob_node_address" {
  ...
  for_each       = { for node in try(local.node_policies.nodes, []) : node.id => node if(try(node.oob_address, null) != null || try(node.oob_v6_address, null) != null) && try(local.modules.aci_oob_node_address, true) && var.manage_node_policies }
  node_id        = each.value.id
  pod_id         = try(each.value.pod, local.defaults.apic.node_policies.nodes.pod)
