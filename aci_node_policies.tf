...
module "aci_rbac_node_rule" {
  ...
  for_each = { for node in try(local.node_policies.nodes, []) : node.id => node if length(try(node.security_domains, [])) != 0 && local.modules.aci_rbac_node_rule && var.manage_node_policies }
  node_id  = each.key
